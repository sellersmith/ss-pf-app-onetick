...
function assertArtifact(manifest) {
  ...
  if (typeof manifest.appId !== 'string') {
    throw new Error(`Invalid artifact appId: ${manifest.appId}`)
  }

  assertSafeSegment(manifest.appId, 'appId')

  if (manifest.artifactType !== 'admin-static') {
...
function deploy(argv) {
  ...
  const targetAppName = path.basename(targetAppRoot)
  ...
  if (targetAppName !== manifest.appId) {
    throw new Error(`Target app root "${targetAppName}" does not match artifact appId "${manifest.appId}"`)
  }
