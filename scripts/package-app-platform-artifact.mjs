...
const appRoot = path.join(repoRoot, 'apps/onetick')
...
const appId = 'onetick'
const appName = 'ss-pf-app-onetick'
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!key?.startsWith('--') || !value || value.startsWith('--')) throw new Error(`Invalid argument pair: ${key ?? ''} ${value ?? ''}`.trim())
...
function collectManifestAssets(manifest, entryKey, visited = new Set()) {
  ...
  for (const importKey of entry.imports ?? []) assets.push(...collectManifestAssets(manifest, importKey, visited))
  return assets
}
...
function validateAdminManifest(artifactRoot, manifestRelativePath) {
  ...
  for (const entryKey of Object.keys(manifest)) {
    for (const assetPath of collectManifestAssets(manifest, entryKey)) {
      ...
      if (!fs.existsSync(fullPath)) throw new Error(`Admin manifest references missing asset: ${assetPath}`)
    }
  }
}
...
function transpileFile(sourceRoot, outputRoot, relativePath) {
  ...
  const outputPath = path.join(outputRoot, relativePath.replace(/\.(?:tsx?|jsx?)$/, '.js'))
...
function transpileDirectory(sourceRoot, outputRoot) {
  for (const relativePath of walkFiles(sourceRoot)) transpileFile(sourceRoot, outputRoot, relativePath)
}
...
function createChecksums(artifactRoot) {
  return Object.fromEntries(
    walkFiles(artifactRoot)
      ...
      .sort()
      .map(relativePath => {
        const digest = crypto.createHash('sha256').update(fs.readFileSync(path.join(artifactRoot, relativePath))).digest('hex')
...
function createTarball(artifactRoot, tarballPath) {
  ...
  if (result.status !== 0) throw new Error(`Failed to create artifact tarball at ${tarballPath}`)
...
function packageArtifact(argv) {
  ...
  const adminEntrySource = 'src/admin/runtime-entry.tsx'

  fs.rmSync(outDir, { recursive: true, force: true })
  fs.mkdirSync(outDir, { recursive: true })

  const sourceAdminRoot = path.join(appRoot, 'dist/admin/runtime')
  if (!copyDirectoryIfExists(sourceAdminRoot, path.join(outDir, 'admin/runtime'))) {
    throw new Error(`OneTick admin assets are missing at ${sourceAdminRoot}. Run build:admin-runtime first.`)
  }
  ...
  const artifact = {
    ...
    version,
    gitSha,
    sourceRepo: 'sellersmith/ss-pf-app-onetick',
    platformCompatibility: { minPageFlyAppPlatform: '0.1.0' },
    admin: { manifest: adminManifest, entrySource: adminEntrySource },
