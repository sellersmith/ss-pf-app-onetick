...
const apps = {
  onetick: {
    ...
    adminAssetDirs: ['runtime'],
    buildCommand: 'npm run build:admin-artifact',
    entryManifest: 'admin/runtime/manifest.json',
    sourceRepo: 'sellersmith/ss-pf-app-onetick',
    sourceAdminRoot: path.join(repoRoot, 'apps/onetick/dist/admin'),
  },
  tailorkit: {
    artifactType: 'admin-static',
    adminAssetDirs: ['copied-routes'],
    buildCommand: 'npm run build:admin-artifact',
...
function assertSafeRelativePath(relativePath) {
  if (typeof relativePath !== 'string' || relativePath.length === 0) {
    throw new Error(`Invalid app-platform admin artifact path: ${relativePath}`)
  }

  const normalized = path.posix.normalize(relativePath)

  if (path.isAbsolute(relativePath) || normalized === '..' || normalized.startsWith('../') || normalized.includes('\0')) {
    throw new Error(`Invalid app-platform admin artifact path escapes artifact root: ${relativePath}`)
  }
  return normalized
}
  ...
  for (const entryKey of Object.keys(manifest)) {
    for (const assetPath of collectManifestAssets(manifest, entryKey)) {
      ...
      if (!fs.existsSync(fullPath)) {
        throw new Error(`App-platform admin artifact manifest references missing asset: ${assetPath}`)
      }
    }
  }
}
...
function packageArtifact(argv) {
  ...
  const sourceFontsRoot = args['source-fonts-root'] || app.sourceFontsRoot
    ? path.resolve(repoRoot, args['source-fonts-root'] ?? app.sourceFontsRoot)
    : null
  ...
  for (const adminAssetDir of app.adminAssetDirs) {
    const sourceAssetRoot = path.join(sourceAdminRoot, adminAssetDir)
    const sourceManifestPath = path.join(sourceAssetRoot, 'manifest.json')

    if (!fs.existsSync(sourceAssetRoot)) {
      throw new Error(`App-platform admin assets for ${appId} are missing at ${sourceAssetRoot}. Run ${app.buildCommand} first.`)
    }

    if (!fs.existsSync(sourceManifestPath)) {
      throw new Error(`App-platform admin manifest for ${appId} is missing at ${sourceManifestPath}.`)
    }

    fs.cpSync(sourceAssetRoot, path.join(outRoot, 'admin', adminAssetDir), { recursive: true })
  }

  if (sourceFontsRoot && fs.existsSync(sourceFontsRoot)) {
