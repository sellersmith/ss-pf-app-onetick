...
Private GitHub repo for OneTick app-platform admin artifact build/deploy.

## Goal

Create CI/CD for OneTick admin static artifact independent from PageFly Core build.
...
```txt
<pagefly-public-root>/app-platform/apps/onetick
```
...
- Decision whether Jenkins runs on target host or deploys via mounted path/SSH wrapper.
